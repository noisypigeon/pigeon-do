...
remote_state {
  ...
  config = {
    ...
    # Prefixed with "pigeon.dev" so existing leaves' state keys are
    # unchanged now that root.hcl lives in pigeon.dev/ instead of the repo
    # root (path_relative_to_include() would otherwise drop that path
    # segment and require a state migration) — see
    # docs/adr/0007-multiple-root-directories.md.
